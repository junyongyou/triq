...
class Embeddings(nn.Module):
    ...
    """
    def __init__(self, config, in_channels=3, n_patches=576):
        ...
        self.dropout = Dropout(config.transformer["dropout_rate"])
        self.pooling_big = nn.MaxPool2d(kernel_size=4)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, num_classes=21843, zero_head=False, vis=False, load_transformer_weights=True):
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            ...
            # tmp1 = logits.view(-1, self.num_classes)
            # tmp2 = labels.view(-1)
            # loss = F.cross_entropy(result, labels)
            # loss = F.kl_div(result, labels)
            loss = categorical_cross_entropy(result, labels)
            # loss = -(labels * result).sum(-1).mean()
...
CONFIGS = {
    ...
    'R50-ViT-Simple': configs.get_r50_simple_config(),
